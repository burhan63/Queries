...
AND   IPT.CLASS_CODE IN ('05')
AND IA.PERIOD_TO_DATE >=SYSDATE
AND IA.ASSORTED_CODE IN (SELECT IAD.ASSORTED_CODE
                         FROM INS_ASSORTED_DETAIL IAD, INS_ASSORTED IAA
                         WHERE TYPE_CODE ='03'
                         AND  IAD.ASSORTED_CODE = IAA.ASSORTED_CODE
                         AND  IAA.PERIOD_TO_DATE>SYSDATE
                         GROUP BY IAD.ASSORTED_CODE
                         HAVING  SUM(SUB_INSURED*co_takaful_contribution_pct(IAD.ASSORTED_CODE))>=100000000
                         )
);
